...
const DISCOURAGED: Rule[] = [
  ...
  },
  {
    ...
    instead: "NEWID() as a DEFAULT; values come from crypto.randomUUID() in the app",
  },
  {
    id: "alter-database",
...
const SQL_ANCHOR =
  /\b(SELECT|INSERT\s+INTO|UPDATE|DELETE\s+FROM|CREATE\s+TABLE|CREATE\s+INDEX|ALTER\s+TABLE|DROP\s+TABLE|FROM|WHERE|VALUES|ORDER\s+BY|GROUP\s+BY|JOIN|BEGIN\s+TRAN|COMMIT\s+TRAN|ROLLBACK\s+TRAN|TRUNCATE)\b/i;

type Finding = {
  file: string;
  line: number;
  cls: "FORBIDDEN" | "DISCOURAGED";
  rule: Rule;
  excerpt: string;
};
...
function runScan(): number {
  ...
  console.error("");
  report(findings);
  const forbidden = findings.filter((f) => f.cls === "FORBIDDEN").length;
  console.error(
    `\n${findings.length} finding(s): ${forbidden} FORBIDDEN (after the ` +
      `2008 R2 floor), ${findings.length - forbidden} DISCOURAGED (available, ` +
      `rejected by design).`,
  );
  return 1;
}
...
function selftest(): number {
  ...
  for (const rule of [...FORBIDDEN, ...DISCOURAGED]) {
  ...
  for (const [id, sample, shouldMatch] of DISCRIMINATION) {
    const rule = [...FORBIDDEN, ...DISCOURAGED].find((r) => r.id === id);
    if (!rule) {
  ...
  console.log(
    `selftest -- ${checks.length - failed.length}/${checks.length} checks passed ` +
      `(${FORBIDDEN.length} forbidden rules, ${DISCOURAGED.length} discouraged rules)`,
  );
